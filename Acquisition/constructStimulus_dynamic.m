function [stimTimes, stim, duration] = constructStimulus_color(A_,B_,nblks,oDur,lrnBlks)
...
od = {A_ B_};
odors = {'Air' 'Air'};
conc = [0.15 0.15];                  % proportion saturated vapor          % 150119 - balancing concs:                                                               % 2U = [0.14 0.15]
...
odorDur = [repmat(15,1,lrnBlks*2),repmat(oDur,1,(nblks*2)),repmat(15,1,lrnBlks*2)];
isi = repmat([15,6],1,nblks);
isi = [0,repmat(6,1,lrnBlks*2-1),60,isi(2:end),30,repmat(6,1,lrnBlks*2-1)];
%odorDur = [60];
