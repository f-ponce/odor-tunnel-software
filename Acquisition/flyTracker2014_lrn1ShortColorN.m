...
Cstate = colorMake(true,nBlks);

% Begin main script

[stimTimes, stim, duration] = constructStimulus_color(AlabeL,BlabeL,nBlks,oDur);
...
while toc < duration
    ...
    if stimTimes(ceil(toc))                  % If this is an odor period...
        ...
        if etime(clock, odorTime) >= chargeTime-5 & etime(clock, odorTime) <= chargeTime-3 & block == (nBlks*2)+2
            if fTme
            toc_ = toc;
            fTme = false;
            end
        ...
        if etime(clock, odorTime) >= chargeTime
            ...
            if occtt == true;
                ...
                if bt ~= (nBlks*2)+2 & bt ~= 1 & rem(bt-1,2) == sideDeterminant
                %if bt == 1 
                    blockn = true;
                else
                    blockn = false;
