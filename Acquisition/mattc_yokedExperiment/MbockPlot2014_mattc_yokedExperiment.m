function flyTracks = MbockPlot2014_mattc_yokedExperiment(flyTracks)
%
% To do:
...
for k = 1:flyTracks.nFlies
    ...
    plot(flyTracks.headLocal(:,2,k),10*flyTracks.relTimes,'k')
    
    plot([0 0], [0 max(10*flyTracks.relTimes)], '--b')
    ...
    text(flyTracks.tunnels(4,k)/2, -5, [ ...
        sprintf('%0.2f', flyTracks.occupancy(k)) ' (' ...
...
for i = idx
    ...
    eventTimes = flyTracks.stim{2,i}(1:end);
    ...
    for ii=1:size(eventTimes,1)
        ...
        eventOnFrame = find(round(10*flyTracks.relTimes) == eventOn, 1);
        eventOffFrame = find(round(10*flyTracks.relTimes) == eventOff, 1);
        ...
        if strcmp(flyTracks.stim{4,i}(1),'Air')
            fcol=[1 1 1];
        elseif strcmp(flyTracks.stim{4,i}(1),'MCH')
            fcol=[0.7 1 0.7];
        elseif strcmp(flyTracks.stim{4,i}(1),'OCT')
            fcol=[1 0.7 1];
        end
        set(ptch1,'edgecolor','none','facecolor', fcol)
        
        ptch2=patch([corridorPos(end), flyTracks.tunnels(4,k), flyTracks.tunnels(4,k), corridorPos(end)],...
            [eventOn,eventOn,eventOff,eventOff], 0);
        set(ptch2,'edgecolor','none','facecolor',fcol)
