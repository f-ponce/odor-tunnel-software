function flyTracks = calculateChoiceStats_mattc_yokedExperiment(flyTracks, refOdor)
% To do:
...
for i = 1:size(flyTracks.stim,2)
    ...
    odorIdx = [odorIdx flyTracks.stim{2,i}(1:end) flyTracks.stim{2,i}(end) + 1];
    
    if find(strncmp(refOdor, flyTracks.stim{4,i}, 3)) == 1
        refOdorOnSideA = [refOdorOnSideA ones(1, length(flyTracks.stim{2,i}(1:end)) + 1)];
    else
        refOdorOnSideA = [refOdorOnSideA zeros(1, length(flyTracks.stim{2,i}(1:end)) + 1)];
    end
