function flyTracks = flyTracker2014_lrn1ShortColorN(occtt,switchSides,nBlks,oDur)
...
addDigitalChannel(sess,'Dev2','Port0/Line2','OutputOnly');
...
Cstate = colorMakeDynamic(nBlks,lrnBlks);

% Begin main script

[stimTimes, stim, duration] = constructStimulus_dynamic(AlabeL,BlabeL,nBlks,oDur,lrnBlks);
...
while toc < duration
    ...
    if stimTimes(ceil(toc))                  % If this is an odor period...
        ...
        if ~finalValveState && etime(clock, odorTime) >= chargeTime
            ...
            %finalValveState = ~finalValveState;
            outputSingleScan(sess,Cstate(block,:))
            tea1 = toc;
        end
        if etime(clock, odorTime) >= chargeTime-5 & etime(clock, odorTime) <= chargeTime-3 & block == (nBlks*2)+lrnBlks*2+1
            if fTme
            toc_ = toc;
            fTme = false;
            end
            if rem(toc-toc_,0.5) < 0.3
            outputSingleScan(sess,[0,0,0,0,0])
            shk = true;
            else
                outputSingleScan(sess,[0,0,0,0,1])
                shk = false;
        ...
        if etime(clock, odorTime) >= chargeTime
            bt = block;
            if switchSides
                sideForShk = rem(bt-1,2) == 0;
            else
                sideForShk = rem(bt,2) == 0;
            end 
            if occtt == true;
                %                 if bt == 2||bt == 3||bt == 4||bt == 5
                %                     blockn = true;
                %                 else
                %                     blockn = false;
                %                 end
                %                 clky = [4,1];
                %             else
                %if bt == 3||bt == 5||bt == 7||bt == 1||bt == 2||bt == 4
                if bt ~= lrnBlks*2+(nBlks*2)+[1:lrnBlks*2] & bt ~= [1:lrnBlks*2] & sideForShk
                %if bt == 1 
                    blockn = true;
                else
                    blockn = false;
 ...
 %                           if ttummy
                                outputSingleScan(sess,[Cstate(bt,1:4),0])
%                            end
                            pause(1/20)
                            %pause(1/30)
                            outputSingleScan(sess,[Cstate(bt,1:4),1])
                            shk = true;
                        else
                            outputSingleScan(sess,[Cstate(bt,1:4),1])
                            shk = false;
