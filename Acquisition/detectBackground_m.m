...
vid.ROIPosition = [20 140 600 213];
...
try
    ...
    blankBg = uint8(zeros(215,600));
end
blankBg = blankBg(141:353,21:620);
...
%         % tunnel intensity
        lb = 5; %27; % tunnel area lower bound
        ub = 22; %40; % tunnel area upper bound
...
%     load tunnel_m
    ...
    for i = 1:length(tunnel)
        ...
        for ii = 1:length(p)
            ...
            if p(ii).Area > 10  % Flies must have area greater than 3
    ...
    if ct == 2
       ...
       bbb = true;
        ...
        for i = toUpdate
            ...
            if pdist([current; pcen(i).initial]) > 25 % current fly centroid > 20 pixels away from original
