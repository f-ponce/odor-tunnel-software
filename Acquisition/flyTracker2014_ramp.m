function flyTracks = flyTracker2014_ramp
...
% Begin main script
[stimTimes, stim, duration] = constructStimulus_r(chargeTime); % Stimlus
                                                           % timecourse
                                                           % for experiment
...
sections = 0.16/(sum(stimTimes)/2);
sections2 = 0.09/(sum(stimTimes)/2);
...
while toc < duration
    ...
    if stimTimes(ceil(toc))                  % If this is an odor period...
        ...
        if odorPeriod
if etime(clock, odorTime) < 150
            mconc =  conc(1) + (sections*floor(etime(clock, odorTime)));
            modconc = [mconc,conc(2)];
            presentOdor_r(valves, modconc);
else
            mconc2 =  conc(2) - (sections2*floor(etime(clock, odorTime)));
            modconc = [mconc,mconc2];
            presentOdor_r(valves, modconc);
end
        end
