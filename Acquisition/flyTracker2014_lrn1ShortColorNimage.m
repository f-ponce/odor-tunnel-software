function flyTracks = flyTracker2014_lrn1ShortColorNimage(occtt,switchSides,nBlks,oDur)
...
Cstate = colorMake(switchSides,nBlks);
    ...
    if stimTimes(ceil(toc))                  % If this is an odor period...
        ...
        if etime(clock, odorTime) >= chargeTime
            ...
            if occtt == true;
                ...
                if bt ~= (nBlks*2)+2 & bt ~= 1 & rem(bt-1,2) == 0
                %if bt == 1 
                    blockn = true;
                else
                    blockn = false;
