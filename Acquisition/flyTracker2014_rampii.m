function flyTracks = flyTracker2014_rampii(odor_stim)
...
% Begin main script
[stimTimes, stim, duration] = constructStimulus_ramp(odor_stim); % Stimlus
                                                           % timecourse
                                                           % for experiment
...
sections = 0.18/(sum(stimTimes));
sections2 = 0.13/(sum(stimTimes));
...
while toc < duration
    ...
    if stimTimes(ceil(toc))                  % If this is an odor period...
        ...
        if odorPeriod
            if odor_stim == 'MCH'
                        mconc =  conc(1) + (sections*floor(etime(clock, odorTime)));
                        modconc = [mconc,conc(2)];
                        presentOdor_r(valves, modconc);
            elseif odor_stim == 'OCT'
                    mconc =  conc(2) + (sections2*floor(etime(clock, odorTime)));
                        modconc = [conc(1),mconc];
                        presentOdor_r(valves, modconc); 
            end
