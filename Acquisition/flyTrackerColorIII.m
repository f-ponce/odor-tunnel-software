function [flyTracks,stimOut] = flyTrackerColor(classical,swappO,btw,revass)
...
% Begin main script
[stimTimes, stim, duration] = constructStimulus_colorIII(AlabeL,BlabeL,classical,swappO,stm); % Stimlus
% timecourse
% for experiment
                ...
                if blockn == true
                    ...
                    if tea2 - tea1 > 2
                        if rem((toc),clky(1)) < clky(2)
                                outputSingleScan(sess,[1,0,1,0,0])
                            pause(1/20)
                            outputSingleScan(sess,[1,0,1,0,1])
                            shk = true;
                        else
