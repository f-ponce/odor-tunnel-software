function [stimTimes, stim, duration] = constructStimulus_m(A_,B_)
...
% isi = [3,3,3,10,10,10];
odorDur = [120,35,35,35,35,35,35,120];                        % in sec
isi = [0,60,15,15,15,15,15,15];                            % in sec
nBlocks = 8;                         % number of odor blocks

preTime = [10];                       % wait time before first odor block
...
%OdorAonTop = repmat(randperm(2)-1, [1, ceil(nBlocks/2)]); % Logical vector
                                                          ...
                                                          % odors{1} on top
                                                          % (Side B)
OdorAonTop = [false(1,nBlocks)];                                                        % KH141217 - hardcode this for consistency
