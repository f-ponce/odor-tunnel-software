function [pre_r,post_r,gen1,preii,postii] = qwikOproc(flyTracks)
pree = [];postt = [];geno = [];
...
post2 = find((flyTracks.relTimes > flyTracks.stim{end-2}(1) & flyTracks.relTimes < flyTracks.stim{end-2}(end)));
pre2 = find((flyTracks.relTimes > flyTracks.stim{26}(1) & flyTracks.relTimes < flyTracks.stim{26}(end)));
post = find((flyTracks.relTimes > flyTracks.stim{22}(1) & flyTracks.relTimes < flyTracks.stim{22}(end)));
...
for i = 1:flyTracks.nFlies
    ...
    sidee{3,1} = nansum(flyTracks.headPosition(pre2,2,i) < ll);
    sidee{3,2} = nansum(flyTracks.headPosition(pre2,2,i) > mm);
    
    sidee{4,1} = nansum(flyTracks.headPosition(post2,2,i) < ll);
    sidee{4,2} = nansum(flyTracks.headPosition(post2,2,i) > mm);
    %Compare odorr with stim sides to find opposite of where
    %shock is
    ccntt = 1;
    for odoc = [flyTracks.stim{4}(1),flyTracks.stim{24}(1),flyTracks.stim{28}(1),flyTracks.stim{end}(1)]
