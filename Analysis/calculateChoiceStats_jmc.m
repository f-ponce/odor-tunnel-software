function flyTracks = calculateChoiceStats_jmc(flyTracks, refOdor)
% To do:
...
for i = 1:size(flyTracks.stim,2)
    % Must be changed in corresponding calcOccupancyStats
    
    % 1 sec added after valve closing
    odorIdx = [odorIdx flyTracks.stim{2,i}(flyTracks.chargeTime:64) flyTracks.stim{2,i}(end) + 1];
    
    if find(strncmp(refOdor, flyTracks.stim{4,i}, 3)) == 1
        refOdorOnSideA = [refOdorOnSideA ones(1, length(flyTracks.stim{2,i}(flyTracks.chargeTime:64)) + 1)];
    else
        refOdorOnSideA = [refOdorOnSideA zeros(1, length(flyTracks.stim{2,i}(flyTracks.chargeTime:64)) + 1)];
    end
