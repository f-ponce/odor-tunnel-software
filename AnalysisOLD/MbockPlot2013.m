...
corridorSize = 17; % in mm (formerly 8mm)
...
if doPlot
    ...
    for k = 1:flyTracks.nFlies
        ...
        makePatches(flyTracks,k,corridorPos,refOdor);
        hold on
        
        plot(flyTracks.centroidLocal(:,2,k),flyTracks.etimes,'k')
        
        plot([0 0], [0 max(flyTracks.etimes)], '--b')
        plot([flyTracks.tunnels(4,k) flyTracks.tunnels(4,k)], [0 max(flyTracks.etimes)], '--b')
        
        % plot the exit points, colored by toward or away from ref odor
        tmp = find(allExits(:,k));

        if strcmp(refOdor, flyTracks.stim{4,1}(1))                          % Fix this to handle muliple pulses
            plot(flyTracks.centroidLocal(tmp(sideAexit{k}),2,k), flyTracks.etimes(tmp(sideAexit{k})), '*g')
            plot(flyTracks.centroidLocal(tmp(~sideAexit{k}),2,k), flyTracks.etimes(tmp(~sideAexit{k})), '*m')
        else
            plot(flyTracks.centroidLocal(tmp(sideAexit{k}),2,k), flyTracks.etimes(tmp(sideAexit{k})), '*m')
            plot(flyTracks.centroidLocal(tmp(~sideAexit{k}),2,k), flyTracks.etimes(tmp(~sideAexit{k})), '*g')
        end
        ...
        text(flyTracks.tunnels(4,k)/2, -5, [ ...
            sprintf('%0.2f', probA(k)) ' (' ...
            sprintf('%2.0f', length(refOdorChosen{k})) ')'], ...
...
function makePatches(flyTracks, k, corridorPos, refOdor)
...
flyTracks.stim2 = temp;

for i = idx
    
    eventTimes = flyTracks.stim2{2,i}(flyTracks.chargeTime:end);
    eventLabels{i} = flyTracks.stim2{1,i};
    
    if strcmp(refOdor, flyTracks.stim{4,i}(1));
        cols = {'g' 'm'};
    else
        cols = {'m' 'g'};
    end
    ...
    for ii=1:size(eventTimes,1)
        ...
        ptch=patch([0, corridorPos(1), corridorPos(1), 0],...
            [eventOn,eventOn,eventOff,eventOff], 0);
        set(ptch,'edgecolor','none','facecolor',cols{1}, 'faceAlpha', 0.3)
        
%         ptch=patch([corridorPos(1), corridorPos(end), corridorPos(end), corridorPos(1)],...
%             [eventOn,eventOn,eventOff,eventOff], 0);
%         set(ptch,'edgecolor','none','facecolor', [0.5 0.5 0.5], 'faceAlpha', 0.5)
        
        ptch=patch([corridorPos(end), flyTracks.tunnels(4,k), flyTracks.tunnels(4,k), corridorPos(end)],...
            [eventOn,eventOn,eventOff,eventOff], 0);
        set(ptch,'edgecolor','none','facecolor',cols{2}, 'faceAlpha', 0.3)
